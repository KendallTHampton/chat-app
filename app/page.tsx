...
export default function Home() {
  ...
  return (
    <div className='h-[100vh] pt-[80px] flex flex-col items-center justify-between bg-neutral-900 gap-4' >

      <div className='w-3/4 h-full rounded-lg p-4 overflow-y-scroll flex flex-col items-start scroll '>
          ...
          <div key={index} className={`mb-2 flex w-full ${message.type === 'user' ? 'justify-start' : 'justify-end'}`}>
            ...
            <div className='flex items-center gap-2 max-w-2/5'>
              ...
              <p
                ...
                className={`border-1 p-3 rounded-lg max-w-sm
      ...
      <div className='w-1/2 mb-20 mt-5'>
        <form className='flex flex-row items-center gap-2' onSubmit={handleSubmit}>
          <input
            className='p-2 border-2 opacity-90 text-black rounded-lg w-full'
            type="text"
            value={inputValue}
            onChange={(e) => setInputValue(e.target.value)}
            placeholder='Type Your Message Here' />
          <button
            className='bg-emerald-600 text-white p-2 rounded-lg w-auto'
